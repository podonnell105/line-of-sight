...
def main():
    ...
    for idx, addr in close_addr_gdf.iterrows():
        ...
        addr_wgs84 = gpd.GeoSeries([addr.geometry], crs=metric_crs).to_crs(epsg=4326)[0]
        rail_wgs84 = gpd.GeoSeries([nearest_rail_pt], crs=metric_crs).to_crs(epsg=4326)[0]
        ...
        if idx % 10 == 0 or idx == close_addr_gdf.index[-1]:
            print(f"Address {idx}: ({addr.geometry.x:.6f}, {addr.geometry.y:.6f}) - LOS: {'Blocked' if score == 0 else 'Clear'}")
    ...
    clear_los_gdf[['los_score', 'distance_to_rail_m', 'geometry'] + [col for col in clear_los_gdf.columns if col not in ['los_score', 'distance_to_rail_m', 'geometry']]].to_csv(clear_los_csv, index=False)
    print(f"Saved {len(clear_los_gdf)} addresses with clear LOS to {clear_los_csv}")
    print(f"Clear LOS percentage: {len(clear_los_gdf)/len(close_addr_gdf)*100:.1f}%")
    ...
    plt.savefig('output/address_los_scores_lidar.png', dpi=300, bbox_inches='tight')
